...
export async function GET(req: NextRequest) {
  ...
  if (region   !== "all") query["survivor.region"]    = { $regex: region, $options: "i" };
  ...
  if (search) {
    query.$or = [
      ...
      { "survivor.region":        { $regex: search, $options: "i" } },
    ];
  }
  ...
  if (region   !== "all") filterParts.push(`Region: ${region}`);
  ...
  const rows = incidents.map((inc, i) => {
    ...
    return `
      <tr style="background:${i % 2 === 0 ? "#fff" : "#f9f9f9"}">
        ...
        <td style="padding:8px 10px;font-size:11px">${sur.region || "—"}</td>
...
<head>
  ...
  <style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .stats { display:flex; gap:16px; padding:16px 32px; background:#f8f9fa; border-bottom:1px solid #eee; flex-wrap:wrap; }
    .stat { text-align:center; }
    .stat-num { font-size:20px; font-weight:700; color:#254252; }
    .stat-label { font-size:10px; color:#888; margin-top:2px; }
    .table-wrap { padding:24px 32px; }
    table { width:100%; border-collapse:collapse; }
    th { background:#254252; color:white; text-align:left; padding:10px; font-size:10px; font-weight:600; text-transform:uppercase; letter-spacing:0.05em; }
...
<body>
  ...
  <div class="confidential">
    🔒 <strong>Confidential</strong> — This document contains sensitive survivor data. Handle in accordance with Rights 4 Her Uganda data protection policies.
  </div>
  <div class="stats">
    ...
    <table>
      <thead>
        <tr>
          ...
          <th>Violence Type(s)</th><th>Urgency</th><th>Status</th><th>Case Officer</th>
        </tr>
      </thead>
